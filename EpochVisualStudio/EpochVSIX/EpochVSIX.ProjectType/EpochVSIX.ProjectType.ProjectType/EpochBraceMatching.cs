...
namespace EpochVSIX
{
    internal class EpochBraceMatchingTagger : ITagger<TextMarkerTag>
    {
        ...
        internal EpochBraceMatchingTagger(ITextView view, ITextBuffer sourceBuffer)
        {
        ...
        public IEnumerable<ITagSpan<TextMarkerTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            if (m_braceList.ContainsKey(currentText))   //the key is the open brace
            {
                ...
                if (EpochBraceMatchingTagger.FindMatchingCloseChar(currentChar, currentText, closeChar, View.TextViewLines.Count, out pairSpan) == true)
                {
            ...
            else if (m_braceList.ContainsValue(lastText))    //the value is the close brace, which is the *previous* character 
            {
                var open = from n in m_braceList
                           ...
                           select n.Key;
                if (EpochBraceMatchingTagger.FindMatchingOpenChar(lastChar, (char)open.ElementAt<char>(0), lastText, View.TextViewLines.Count, out pairSpan) == true)
                {
