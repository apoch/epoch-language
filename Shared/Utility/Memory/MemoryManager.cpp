...
UnalignedMemoryAllocator::~UnalignedMemoryAllocator()
{
	for(std::vector<AllocBlock>::iterator iter = AllocBlocks.begin(); iter != AllocBlocks.end(); ++iter)
		::HeapFree(TheGlobalHeap.GetHeap(), 0, iter->Storage);
}
...
{
	size_t numpages = (numbytes / TheGlobalHeap.GetPageSize()) + 1;

	void* memory = ::HeapAlloc(TheGlobalHeap.GetHeap(), 0, numpages * TheGlobalHeap.GetPageSize());
	if(!memory)
