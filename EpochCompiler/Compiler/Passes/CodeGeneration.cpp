...
namespace
{
	...
	{
		std::set<StringHandle> explicitnativetags;
		...
		for(std::map<StringHandle, IRSemantics::Structure*>::const_iterator iter = structures.begin(); iter != structures.end(); ++iter)
		{
			...
			explicitnativetags.insert(iter->second->GetConstructorName());
			explicitnativetags.insert(iter->second->GetAnonymousConstructorName());
			explicitnativetags.insert(iter->second->GetCopyConstructorName());
		}
		...
		for(std::set<StringHandle>::const_iterator iter = explicitnativetags.begin(); iter != explicitnativetags.end(); ++iter)
		{
			emitter.TagData(*iter, L"native", std::vector<std::wstring>());
		}
